#!/usr/bin/env bash
#
# Version consistency gate for the KnishIO Rust SDK.
#
...
# Cargo.toml's [package] version. Line-anchored so it matches the package key and not
# `rust-version` or any dependency's inline `version = `.
collect "Cargo.toml [package] version" \
  "$(sed -n 's/^version *= *"\([0-9][^"]*\)".*/\1/p' Cargo.toml | head -1)"
# -----------------------------------------------------------------------------------------
