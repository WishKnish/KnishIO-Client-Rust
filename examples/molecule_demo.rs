use knishio_client::{Wallet, Molecule, types::MetaItem};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut sender_wallet = Wallet::create(Some(secret), None, "TEST", None, None, None)?;
    sender_wallet.balance = "1000.0".to_string();
    println!("   Sender wallet: {}", sender_wallet.address.as_ref().unwrap());
    println!("   Sender balance: {}", sender_wallet.balance);
    
    let recipient_wallet = Wallet::create(Some("different-secret"), None, "TEST", None, None, None)?;
